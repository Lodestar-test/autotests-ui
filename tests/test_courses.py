...
@pytest.mark.coursesё
@pytest.mark.regression
def test_empty_courses_list(courses_list_page: CoursesListPage):
    courses_list_page.vizit('https://nikita-filonov.github.io/qa-automation-engineer-ui-course/#/courses')
    courses_list_page.navbar.check_visible('username')  # проверяем отображение компонента Navbar
    courses_list_page.sidebar.check_visible()   # проверяем отображение компонента Sidebar
    courses_list_page.check_visible_courses_title()  # проверяем отображение заголовка "Courses"
    courses_list_page.check_visible_create_course_button()  # проверяем отображение кнопки создания курса
